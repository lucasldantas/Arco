...
# Verifica se qualquer variação de FINDME apareceu (com hífens ou underlines)
if lpstat -v | grep -Ei "PRINT[-_]ARCO[-_]FINDME" > /dev/null; then
    echo "SUCESSO: Impressora FINDME detectada no sistema!"
    
    echo "STATUS: Iniciando remoção das impressoras legadas..."
    
    # Lista de alvos conforme solicitado (o script tratará - como [-_])
    LEGACY_LIST=(
    ...
    for TARGET in "${LEGACY_LIST[@]}"; do
        # Transforma hífens do nome em um padrão que aceita hífen ou underline
        # Ex: PRINT-ARCO vira PRINT[-_]ARCO
        PATTERN=$(echo "$TARGET" | sed 's/-/[-_]/g')
        
        # Filtra a impressora instalada que casa com o padrão E não tem FINDME no nome
        MATCH=$(echo "$INSTALLED_PRINTERS" | grep -E "^${PATTERN}$" | grep -v "FINDME")

        if [ -n "$MATCH" ]; then
            for PRINTER in $MATCH; do
                echo "REMOVENDO: $PRINTER"
                sudo lpadmin -x "$PRINTER"
            done
        else
            echo "IGNORADO: $TARGET não encontrada ou protegida."
        fi
    done
